			    ...
			    const struct wpabuf *msg, size_t *plen)
{
	...
	if (wpabuf_len(msg) < sizeof(*hdr)) {
		dbg_printf(MSG_INFO, "EAP: Too short EAP frame");
		return NULL;
	}

	len = be_to_host16(hdr->length);
	if (len < sizeof(*hdr) + 1 || len > wpabuf_len(msg)) {
		dbg_printf(MSG_INFO, "EAP: Invalid EAP length");
		return NULL;
	}
	...
	if (*pos == EAP_TYPE_EXPANDED) {
		...
		u32 exp_type;
		if (len < sizeof(*hdr) + 8) {
			dbg_printf(MSG_INFO, "EAP: Invalid expanded EAP "
				   "length");
			return NULL;
		}
		pos++;
		exp_vendor = bytes_to_be24(pos);
		pos += 3;
		exp_type = bytes_to_be32(pos);
		pos += 4;
		if (exp_vendor != vendor || exp_type != (u32) eap_type) {
			dbg_printf(MSG_INFO, "EAP: Invalid expanded frame "
				   "type");
			return NULL;
		}
		...
		*plen = len - sizeof(*hdr) - 8;
		...
		if (vendor != EAP_VENDOR_IETF || *pos != eap_type) {
			dbg_printf(MSG_INFO, "EAP: Invalid frame type");
			return NULL;
		}
